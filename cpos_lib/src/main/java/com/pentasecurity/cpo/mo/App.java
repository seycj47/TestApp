...
import com.pentasecurity.cpo.mo.model.ContractVerifyServiceCode;
import com.pentasecurity.cpo.mo.model.EvChargeEndMsg;
...
import com.pentasecurity.cpo.mo.model.PmaxSchedule;
...
public final class App {
	...
	private void runMainCommand(BufferedReader br) throws Exception {
		...
		while (command.equals("0") == false) {
			...
			System.out.println("7 : Issue CPO Certificate");
			System.out.println("8 : OCSP Request");
			System.out.println("9 : Issue OEM Provisioning Certificate");
			System.out.println("10 : get OEM Provisioning Certificate");
			System.out.println("11 : get CPO Certificate");
			...
			if (command.equals("1")) {
				...
				runCommandIssueCpoCertificate(br);
			} else if (command.equals("8")) {
//            	runCommandOcspRequest(br);
			} else if (command.equals("9")) {
				runCommandIssueOemProvCert(br);
			} else if (command.equals("10")) {
				runCommandGetOemProvCert(br);
			} else if (command.equals("11")) {
				runCommandGetCpoCert(br);
			}
		}
	}
	...
	private void runCommandPushChargeSales(BufferedReader br) throws Exception {
		...
		EvChargeEndMsg evChargeEndMsg = new EvChargeEndMsg("CPID112948289",
				...
				new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse("2019-05-05 15:00:00"), "DC", "765", "8000", "10000",
				"0", "0", "0", "card", "0", null, "완충", "0", "0", "40다3502", null, "0", "0", "0", null, "0", "0", "0",
				"0", "0", "0", null, "0", "0", null, "KR-KEP-0DAAKGH9U-5", "0", null, null, null, null, null, "0",
				null);

		ChargeSalesRequest request = new ChargeSalesRequest(emaid, sessionId, evChargeEndMsg, signature, seccCert,
				subca1Cert, subca2Cert);
	...
	private void runCommandIssueCpoCertificate(BufferedReader br) throws Exception {
		System.out.println("input cn > ");
		String cn = br.readLine();
		System.out.println("");

		IssueCpoCertResponse response = this.v2g.issueCpoCertificate(cn);
		System.out.println(Json.instance().toJson(response, false));
	}

	private void runCommandGetCpoCert(BufferedReader br) throws Exception {
		System.out.println("Input cn");
		String cn = br.readLine();
		System.out.println("");
		IssueCpoCertResponse response = this.v2g.getCpoCertByCn(cn);
		System.out.println("******************");
		System.out.println(Json.instance().toJson(response, false));
	}
	...
	private void runCommandGetOemProvCert(BufferedReader br) throws Exception {
		System.out.println("Input PCID");
		String pcid = br.readLine();
		System.out.println("");
		IssueOemProvCertRes issueOemProvCertRes = this.v2g.getOemProvCertByPcid(pcid);
		System.out.println("******************");
		System.out.println(issueOemProvCertRes);
	}
